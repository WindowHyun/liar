...
async function main() {
  ...
  net.sendBroadcastReliable({ type: 'START', tag: 't2', id: 'PEER-A', roundId: 'r1', roster: [] });
  await wait(1600);
  check('[P0-3] 재전송 포함 브로드캐스트 → 수신 1건', countOf('t2') === 1, `수신 ${countOf('t2')}건`);

  // 3. ACK를 받는 유니캐스트 (내 수신 소켓이 127.0.0.1로 받아 ACK를 돌려준다)
  let failed3 = null;
  net.sendUnicastReliable({ type: 'WORD', tag: 't3', to: 'me', word: '사과' }, '127.0.0.1', (why) => { failed3 = why; });
  await wait(1600);
  ...
  let failed4 = null;
  const t0 = Date.now();
  net.sendUnicastReliable({ type: 'WORD', tag: 't4', to: 'nobody' }, '192.0.2.99', (why) => { failed4 = why; });
  await wait(2500);
  ...
  const ret = net.sendUnicast({ type: 'WORD', tag: 't5', word: '남의제시어' }, undefined);
  await wait(600);
