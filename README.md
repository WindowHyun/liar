...
- 진행 순서는 **설명 → 자유 대화 → 투표 → 색출** 이다.
  1. **설명** — 대화권이 랜덤 순서로 한 명씩 돌아간다. **1인 1회**, 말하면 바로 다음 사람에게
     넘어간다. 60초 안에 말하지 않으면 그냥 넘어간다.
  2. **자유 대화(1분)** — 전원이 한 번씩 말하고 나면 열린다. 여기서만 투표를 제안할 수 있고,
     1분이 지나면 찬반 없이 곧바로 투표로 넘어간다.
  3. **투표** — 누구든 **🎧** 버튼을 누르면 전원 화면에 **O / X** 가 뜬다. 찬성이 **절반 이상**이면
     투표로 넘어간다(4명 중 2명 O 2명 X도 50%라서 진행). 부결되면 자유 대화로 돌아간다.
  4. **색출** — 지목된 사람이 라이어가 아니면 라이어 승. 라이어면 30초 안에 제시어를 맞혀야 역전승.
- 찬반(O/X) 단계부터 결과가 날 때까지 대화창이 잠긴다.
- **방 나가기**는 즉시 나간다(자리를 남기지 않는다). 뜻하지 않게 끊긴 경우에만 **10초**
  동안 자리를 남기고, 남은 참가자가 최소 인원보다 적어지면 그 판은 바로 취소된다.
